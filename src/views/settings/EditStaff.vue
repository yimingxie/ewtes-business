      ...
      <div class="content">
        ...
        <el-form :model="editStaffForm" :rules="rules" ref="form" label-width="105px" :hide-required-asterisk='true'>
          <el-row>
            <el-col :span="12">
              <el-form-item label="姓名：" prop="name">
                <el-input v-model="editStaffForm.name" placeholder="请输入姓名" size="small"></el-input>
              </el-form-item>
            </el-col>
          ...
          <el-form-item label="手机号：" prop="account">
            <el-input v-model="editStaffForm.account" placeholder="(将作为为app登录账号)请输入手机号" size="small" maxlength="11" disabled></el-input>
          </el-form-item>
          <el-form-item label="出生日期：" prop="birthday">
            ...
            <el-date-picker type="date"  placeholder="请选择日期" v-model="editStaffForm.birthday" style="width: 248px" size="small"></el-date-picker>
          </el-form-item>
          <el-form-item label="从业日期：" prop="empTime">
            ...
            <el-date-picker type="date"  placeholder="请选择日期" v-model="editStaffForm.empTime" style="width: 248px" size="small"></el-date-picker>
          </el-form-item>
          <el-form-item label="入职日期：">
            ...
            <el-date-picker type="date"  placeholder="(选填)请选择日期" v-model="editStaffForm.entryTime" style="width: 248px" size="small"></el-date-picker>
          </el-form-item>
          <el-form-item label="部门：" prop="depId">
            <el-select @change="depSelectChange" v-model="editStaffForm.depId" placeholder="请选择部门" size="small">
              <el-option
          ...
          <el-form-item label="管辖区域：" prop="areaProp">
            ...
            <el-dropdown trigger="click"  :hide-on-click="false" class="selectLiftDropdown">
              ...
              <el-dropdown-menu slot="dropdown" class="liftDropdown">
                ...
                <div style="height:25px;padding-right: 10px;margin-bottom: 10px;">
                  <search-input placeholderValue="搜索电梯注册代码" @search="searchLift1" @cancel="searchLift1"></search-input>
                </div>
                <div v-if="getLIftDataFirst.length > 0" v-for="item in getLIftDataFirst" :key="item.elevCode">
                  ...
                  <div class="dropdownArea">{{item.areaName}}</div>
                  <div v-for="list in item.data" :key="list.elevCode">
                    <!-- <span class="dropdownList dropdown1">{{list.regCode}}</span> -->
                    <span class="dropdownList dropdown1" :title="list.elevCode" v-if="list.elevCode.length > 9">{{ list.elevCode.substring(0, 4)}}...{{list.elevCode.substring(list.elevCode.length-4) }}</span>
                    <span class="dropdownList dropdown1" v-else>{{list.elevCode}}</span>
                    <span class="dropdownList dropdown2">{{list.inNum}}</span>
                    <span class="dropdownList dropdown3">{{list.address}}</span>
                    <span class="dropdownList dropdown4">
                      <el-checkbox-group v-model="checkedLiftAs" @change="handleCheckedLiftAsChange">
                        <el-checkbox :label="list.elevCode" :key="list.elevCode" class="checkbox16" :disabled="getCheckDisabled1(list.elevCode)">{{nonetext}}</el-checkbox>
                      </el-checkbox-group>
                    </span>
                  </div>
          ...
          <el-form-item >
            ...
            <el-dropdown trigger="click" :hide-on-click="false" class="selectLiftDropdown">
              ...
              <el-dropdown-menu slot="dropdown" class="liftDropdown">
                <!-- 搜索栏 -->
                <div style="height:25px;padding-right: 10px;margin-bottom: 10px;">
                  <search-input placeholderValue="搜索电梯注册代码" @search="searchLift2" @cancel="searchLift2"></search-input>
                </div>
                <div v-if="getLIftDataSecond.length > 0" v-for="item in getLIftDataSecond" :key="item.elevCode">
                  <div class="dropdownArea">
                    {{item.areaName}}
                    <!-- <el-checkbox v-model="checkAll" @change="checked=>handleCheckAllChange(checked, item.areaName, 2)" class="checkbox16">{{nonetext}}</el-checkbox> -->
                  ...
                  <div v-for="list in item.data" :key="list.elevCode">
                    ...
                    <span class="dropdownList dropdown3">{{list.address}}</span>
                    <span class="dropdownList dropdown4">
                      <el-checkbox-group v-model="checkedLiftBs" @change="value=>handleCheckedLiftBsChange(value, list.areaName)">
                        <el-checkbox :label="list.elevCode" :key="list.elevCode" class="checkbox16" :disabled="getCheckDisabled2(list.elevCode)">{{nonetext}}</el-checkbox>
                      </el-checkbox-group>
                    </span>
                  </div>
...
export default {
  data() {
    var checkName = (rule, value, callback) => {
      ...
      if (len > 20) {
          //重点重点，下面就是填写提示的文字
          callback(new Error('请控制在20位字符以内'));
      } else {
          callback();
      }
    };
    return {
      ...
      regionOptions: newArea.newAreaOption(),
      areaForm: {
        city : '',
        area : '',
        minarea : '',
  ...
  methods: {
    // 第一负责人、第二负责人管辖电梯不能相同
    getCheckDisabled1(elevCode){
      var flag = false
      // 第二负责人管辖电梯在一负责人选项中设为disabled
      flag = this.checkedLiftBs.some(item =>{
        if(item == elevCode){
            return true;
        }
      })
    ...
    // 第一负责人、第二负责人管辖电梯不能相同
    getCheckDisabled2(elevCode){
      var flag = false
      // 第一负责人管辖电梯在二负责人选项中设为disabled
      flag = this.checkedLiftAs.some(item =>{
        if(item == elevCode){
            return true;
        }
      })
     ...
     // 全选，非全选
    handleCheckAllChange(val,areaName,num) {
      console.log("areaName==" + val)
      console.log("areaName==" + areaName)
      this.checkedAllStaff = []
      // 第二负责人电梯
      if(num == 2){
        var selecLifts = this.getLIftDataSecond.filter(item => {
          return item.areaName == areaName
        })
        selecLifts[0].data.forEach(lift =>{
          this.checkedAllStaff.push(lift.elevCode)
        })
        this.checkedLiftBs = val ? this.checkedAllStaff : [];
    ...
    handleCheckedLiftBsChange(value, areaName){
      ...
      this.checkAll = checkedCount === this.checkedAllStaff.length;
      console.log("checkedLiftBs==="+ JSON.stringify(this.checkedLiftBs))
      // this.isIndeterminate = checkedCount > 0 && checkedCount < this.getAllAccountJson.length;
    },
    handleClick() {
      alert('button click');
    ...
    getFirstData(){
      ...
      if(val.length > 0){
        // for(var i = 0; i < val.length; i++){
          this.getElevatorByAreaParams1.areaCode = val
          api.accountApi.getElevatorByArea(this.getElevatorByAreaParams1).then((res) => {
            if(res.data.code === 200 && res.data.message === 'success'){
               
                // if(res.data.data.elevator.length > 0){
                  this.totalPageSize1 = res.data.data.total
                  res.data.data.elevator.forEach(item =>{
                    firstList.push(item)
                  })
    ...
    getScdData(){
      ...
      this.checkAll = false //全选置false
      // this.checkedLiftBs = []
      var secondList = []
      if(val.length > 0){
        // for(var i = 0; i < val.length; i++){
          this.getElevatorByAreaParams2.areaCode = val
    ...
    upLoadHeadPic(fileObj) {
      ...
      api.accountApi.uploadPic(formData).then((res) => {
        if(res.data.code === 200 && res.data.message === 'success'){
          ...
          this.editStaffForm.avatar = res.data.data.fileName
        } else {
    ...
    getAllAccountData(){
      api.accountApi.getStaffDetails(this.$route.params.staffId).then((res) => {
        if(res.data.code === 200 && res.data.message === 'success'){
          ...
          this.editStaffForm.account = this.getStaffInfo.username//员工姓名
          this.editStaffForm.depId = this.getStaffInfo.depId//部门id
          console.log(" ++++::"+ this.editStaffForm.depId)
          this.editStaffForm.gender = this.getStaffInfo.gender// 性别
          

          this.editStaffForm.empUrl = this.getStaffInfo.empUrl == null ? '' : this.getStaffInfo.empUrl// 从业资格证 图片地址
          this.editStaffForm.avatar = this.getStaffInfo.avatar  == null ? '' : this.getStaffInfo.avatar// 头像地址

          this.editStaffForm.empTime = this.getStaffInfo.empTime || '' // 从业日期
          this.editStaffForm.entryTime = this.getStaffInfo.entryTime|| ''//入职日期
          this.editStaffForm.birthday = this.getStaffInfo.birthday || ''//入职日期

          console.log("this.editStaffForm.birthday" + this.editStaffForm.avatar)
          
          this.checkList2 = this.getStaffInfo.areaCode
          ...
          if(res.data.data.major){
            ...
            res.data.data.major.forEach(item =>{
              majorArr.push(item.elevCode)
            })

            // this.editStaffForm.majorRegCode = majorArr.join(',')
            this.editStaffForm.majorRegCode = majorArr
          }
          ...
          if(res.data.data.minor){
            ...
            res.data.data.minor.forEach(item =>{
              minorArr.push(item.elevCode)
            })

            // this.editStaffForm.minorRegCode = minorArr.join(',')
            this.editStaffForm.minorRegCode = minorArr
          }
          

          // 员工头像
          if(this.getStaffInfo.avatar){
            this.imageUrl1 = api.accountApi.viewPic(this.getStaffInfo.avatar)
          }
          // 从业资格证
    ...
    depSelectChange(val,type){
      ...
      var selectRange = selectDep[0].area
      ...
      if(this.checkList1.length > 0){
        ...
        for(var i = 0 ; i< this.checkList1.length; i++){
          sheng.push(this.checkList1[i].substring(0,2))
          shi.push(this.checkList1[i].substring(0,4))
          qu.push(this.checkList1[i].substring(0,6))
        }
        // 去重
        sheng = this.uniq(sheng)
        // 去重
        shi = this.uniq(shi)
        console.log("省：" + sheng)
        console.log("市：" + shi)
        console.log("区：" + qu)
        this.regionOptions2 = [] // 置空
        console.log("1111111111----------------------")
        // 构建员工管辖区域数据
        this.regionOptions.forEach((item, i) => {
          for(var shen = 0;shen < sheng.length; shen++) {
            console.log("sheng.length==" + sheng.length)
            // 第一级
            var obj1 = {
              value: '',
              label: '',
            ...
            // 查找对应的省
            if(item.value === sheng[shen]){
              
              obj1.value = sheng[shen]
              ...
              if (item.children) { // 二级循环
                ...
                item.children.forEach((secondItem, secondI) => {
                  for(var shiL = 0;shiL < shi.length; shiL++){
                    // 第二级
                    var obj2 = {
                      value: '',
                      label: '',
        ...
        console.log("selectRange---" + JSON.stringify(this.regionOptions2))
        // 构建员工管辖区域数据 end
    ...
    regionChange2(val, totalLabel) {
      ...
      this.checkAreaList = []
      this.checkedLiftAs = [] //切换管辖区域时 第一负责人电梯置空
      this.checkedLiftBs = [] //切换管辖区域时 第二负责人电梯置空
    ...
    confirmEditAccount(){
      ...
      if(this.checkedLiftAs.length > 0){
        this.editStaffForm.majorRegCode = this.checkedLiftAs // 管辖电梯（第一负责人）
      }
      if(this.checkedLiftBs.length > 0){
        this.editStaffForm.minorRegCode = this.checkedLiftBs // 管辖电梯（第二负责人）
      }

      this.editStaffForm.areaCode = this.checkAreaList // 管辖区域code
      this.editStaffForm.manageArea = this.selectedAreaLabels.join(',') // 管辖区域

      // console.log("this.editStaffForm===" + JSON.stringify(this.editStaffForm))

      if(this.editStaffForm.avatar == '' || this.editStaffForm.avatar == null){
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          // 转时间戳
          this.editStaffForm.birthday = new Date(this.editStaffForm.birthday).getTime()
          this.editStaffForm.empTime = new Date(this.editStaffForm.empTime).getTime()
          this.editStaffForm.entryTime = new Date(this.editStaffForm.entryTime).getTime()
    ...
    // 搜索第一负责人电梯
    searchLift1(key){
      this.getElevatorByAreaParams1.offset = 1
      this.getElevatorByAreaParams1.queryStr = key
      this.getFirstData()
    },
    
    // 搜索第一负责人电梯
    searchLift2(key){
      this.getElevatorByAreaParams2.offset = 1
      this.getElevatorByAreaParams2.queryStr = key
      this.getScdData()
    },
