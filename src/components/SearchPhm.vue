<template>
  <div class="search-container">
    <div class="llcb-search-box">
      <input class="lsearch-input" type="text" v-model="cCode" placeholder="搜索设备注册码/详细地址" @keyup="showList($event)" @keyup.enter="search(cCode)" @blur="show = false">
    ...
    <div class="llcb-search-tips" v-show="show">
      ...
      <div class="search-tip-box" v-for="(item, i) in resultList" :class="i == cur ? 'on' : ''" :key="i" @mousedown="search(item.phmId)">
        <h4>{{item.phmId}}</h4>
...
export default {
  ...
  methods: {
    ...
    showList(e) {
      ...
      api.park.showPhmSearchList(this.cCode).then(res => {
        // console.log('子组件', res.data)
        if (res.data.data) {
          this.resultList = res.data.data
        } else {
          this.resultList = []
      ...
      if(this.resultList.length > 0){
        switch (e.keyCode) {
          ...
          case 13://回车选择
              if (this.cur != -1)
              this.cCode = this.resultList[this.cur].phmId
              // this.search(this.resultList[this.cur].phmId)
              // console.log("aaaaa====" + this.resultList[this.cur].phmId)
              break;
        }
      }
    ...
    search(phmId) {
      ...
      if(this.resultList.length > 0){
        ...
        var liftItem = this.resultList.filter(item => {
          return item.phmId == phmId
        })

        if(liftItem.length > 0) {
          parkIdCopy = liftItem[0].parksId || '暂未绑定园区'
        } else {
          parkIdCopy = '暂未绑定园区'
        }
      ...
      // 传phmId、parkIdCopy至父组件
      this.$emit('childCode', phmId, parkIdCopy)
    },
